...
/*
  ...
  var wordHash = {}
  $.each(group, function(ix, value) {
    var wString = value.val.replace(/[^a-zA-Z]/g, " ")
    var ws = wString.split(/ +/)
    $.each(ws, function(ix, w) {
      if (!(w in wordHash)) {
        wordHash[w] = 0
      }
      wordHash[w]++
  ...
  // Make array for word cloud plugin
  words = [ ]
  $.each(wordHash, function(ix, value) {
    words.push({ text: ix, size: value })
  })
...
*/
  ...
  var cases = {};
  $.each(group, function(ix, value) {
    value.val.split(wordSeparators).forEach(function(word) {
